...
mutable struct LightDark1D{F<:Function} <: POMDPs.POMDP{LightDark1DState,Int,Float64}
    ...
    transition_s::Float64
    sigma::F
end

default_sigma(x::Float64) = abs(x - 10)+1e-2#/sqrt(2) 

LightDark1D() = LightDark1D(0.9, 10.0, -10.0, 1.0, 0.0, 0.1, default_sigma)
...
actions(::LightDark1D) = [-10,-1,0,1,10]
...
initialstate(pomdp::LightDark1D) = LDNormalStateDist(0, 10)
...
function transition(p::LightDark1D, s::LightDark1DState, a::Int)
    if a == 0
        return Deterministic(LightDark1DState(-1, s.y+0))
    else
        #return Deterministic(LightDark1DState(s.status, s.y+a*p.step_size))
        return Deterministic(LightDark1DState(s.status, s.y+a*p.step_size+rand(Normal(0, p.transition_s))))
    end
end
