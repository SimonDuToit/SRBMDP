...
Base.isempty(t::MCTSTree) = isempty(t.s_lables)
...
@inline total_n(n::StateNode) = n.tree.total_n[n.tree.state_map[n.id]]
#@inline child_ids(n::StateNode) = n.tree.child_ids[n.id]
@inline child_ids(n::StateNode) = n.tree.child_ids[n.tree.state_map[n.id]]
...
function build_tree(planner::AbstractMCTSPlanner, s)
    ...
    root = insert_node!(tree, planner, s, 1)

    timer = planner.solver.timer
    start_s = timer()
    # build the tree
    for n = 1:n_iterations
        simulate(planner, root, depth, s)
...
function simulate(planner::AbstractMCTSPlanner, node::StateNode, depth::Int64, state)
    mdp = planner.mdp
    rng = planner.rng
    s = state
    tree = node.tree
    ...
    spid = said+1
    #if spid == 0
    #spid = get(tree.state_map, spid, 0)  
    if get(tree.state_map, spid, 0) == 0
        spn = insert_node!(tree, planner, sp, spid)
        q = r + discount(mdp) * estimate_value(planner.solved_estimate, planner.mdp, sp, depth-1)
    else
        q = r + discount(mdp) * simulate(planner, StateNode(tree, spid) , depth-1, sp)
    end
    if planner.solver.enable_tree_vis
        record_visit!(tree, said, spid)
    end

    tree.total_n[node.tree.state_map[node.id]] += 1
    tree.n[said] += 1
    tree.q[said] += (q - tree.q[said]) / tree.n[said] # moving average of Q value
    return q
end

@POMDP_require simulate(planner::AbstractMCTSPlanner, s, depth::Int64, state) begin
    mdp = planner.mdp
    P = typeof(mdp)
    S = statetype(P)
    A = actiontype(P)
...
function insert_node!(tree::MCTSTree, planner::MCTSPlanner, s, spid::Int64)
    #push!(tree.s_labels, s) 
    push!(tree.child_ids, [])
    tree.state_map[spid] = length(tree.child_ids)
    total_n = 0
    ...
    return StateNode(tree, spid)
end

@POMDP_require insert_node!(tree::MCTSTree, planner::AbstractMCTSPlanner, s, spid::Int64) begin
