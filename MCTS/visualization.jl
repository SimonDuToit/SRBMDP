...
function D3Trees.D3Tree(tree::MCTSTree, root_state=1; title="MCTS tree", kwargs...)
    ...
    for (i,((said,sid),n)) in enumerate(vs)
        ...
        nodes[1+i] = Dict("type"=>:state,
                          "child_d3ids"=>[1+nsas+c for c in tree.child_ids[tree.state_map[sid]]],
                          "tag"=>node_tag(s),
                          "tt_tag"=>tooltip_tag(s),
                          "n"=>n,
                          "total_n"=>tree.total_n[tree.state_map[sid]],
